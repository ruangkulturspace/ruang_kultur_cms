...
const ModalAddType = ({ session, modalAdd, setModalAdd, onFinish = () => { } }) => {
    ...
    const handleOk = () => {
        formUserType.validateFields().then(async values => {
            ...
            var datar = await requestPost(
                session,
                process.env.NEXT_PUBLIC_API_URL + '/api/v1/admin/type/create',
                param
            );
            setLoading(false);
            ...
            >
                <Row gutter={[10, 10]} justify="center" align="middle">
                    <Col xs={24} sm={24} md={24} lg={24}>
                        <h1 className="text-center headerPage">Create Type</h1>
                    </Col>
                </Row>
