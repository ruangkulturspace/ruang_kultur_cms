...
const ModalDeleteBanner = ({ session, modalDelete, setModalDelete, dataDelete, onFinish = () => {} }) => {
    ...
    const FetcherDeleteUser = async () => {
        ...
        var datar = await requestDelete(
            session,
            process.env.NEXT_PUBLIC_API_URL + '/api/v1/admin/banner/delete/' + dataDelete._id
        );
        setLoading(false);
